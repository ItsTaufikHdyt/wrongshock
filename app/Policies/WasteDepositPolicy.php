...
class WasteDepositPolicy
{
    ...
    public function view(User $actor, WasteDeposit $deposit): bool
    {
        return $actor->isPlatformAdmin()
            || ($actor->isBankAdmin() && $actor->wasteBanksAsStaff()->whereKey($deposit->waste_bank_id)->exists())
    ...
    {
        return $actor->isBankAdmin()
            && $actor->wasteBanksAsStaff()->whereKey($deposit->waste_bank_id)->where('status', true)->exists()
